...
char g_data_path[_MAX_PATH];
char g_setup_path[_MAX_PATH];
...
int RAP_IsSaveFile(player_t *a1)
{
    ...
    for (v24 = 0; v24 < 10; v24++)
    {
        ...
        if (v20)
        {
            ...
            fclose(v20);
            if (!stricmp(vb0.f_0, a1->f_0) && !stricmp(vb0.f_14, a1->f_14))
            {
                v1c = 1;
                break;
            }
...
int RAP_LoadWin(void)
{
    ...
    for (v1c = 0; v1c < 10; v1c++)
    {
        ...
        if (!access(v68, 0))
        {
            if (v20 == -1)
                v20= v1c;
            strncpy(v254[v1c], v68, _MAX_PATH);
        }
    }
    if (v20 == -1)
        return-1;
